...
3. **Canopy Cover Density Estimation** : This generates a Canopy Density Classification
    map over India with 3 classes - High, Medium and Low.
   
4. **Ecological Clustering:** This generates ecologically similar clusters in an area of interest
...
2. **LTP_masks_and_derived_rasters.ipynb:** This Python notebook is used to generate
    the following historical tree cover masks, available as GEE assets:
        ...
        i. forested_since_1985: Pixels that have been under tree cover continuously since 1985 to 2023.
       
        ii. forested_since_2000 : Pixels that have been under tree cover continuously since 2000 to 2023.
       
        iii. forested_1985_2000_union: Pixels that were under tree cover at any point in 1985-2000.
       
        iv. forested_2000_onwards_union: Pixels that were under tree cover at any point in 2000-2023.
...
4. **cd_analysis.ipynb** : This notebook is used to merge the maps generated in Steps 1 and
...
3. Do coarse clustering of an ACZ based on rainfall, temperature, coarse elevation (5 km)
...
4. Get FAO soil types features from Harmonized World Soil Database v2 and intersect
    these features with the SNIC superpixels generated in Step 2.
   
5. In each of these intersected features, do K-Means clustering based on fine elevation,
...
6. After generating SNIC and K-Means clusters, use **join_images.js** to mosaic together all
    the ACZ-wise outputs and stack them into a single pan-India multilevel ecological
    clustering asset.
   
7. After obtaining the final clusters, use **assign_potential.js** to generate and save the CCD
    Potential table containing one row for each K-Means cluster.
   
8. To add new features (like rainfall, temperature, pH) to the pipeline, modify
...
9. To visualize new features, add a visparams dictionary to VIS_PARAMS in
    **visualization.js**.
   
10. Helper modules: **kmeans.js** and **snic.js**.
